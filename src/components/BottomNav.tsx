...
const MORE_ITEMS = [
  ...
  { label: 'Tracht', href: '/dashboard/tracht', icon: (
    <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.75" strokeLinecap="round" strokeLinejoin="round">
      <path d="M12 22V12"/><path d="M12 12c0-3 2-5 5-5 0 3-2 5-5 5z"/><path d="M12 12c0-3-2-5-5-5 0 3 2 5 5 5z"/><circle cx="12" cy="8" r="2.5"/>
    </svg>
  )},
      ...
      {showMore && (
        <div className="fixed inset-0 z-40 md:hidden" onClick={() => setShowMore(false)}>
          ...
          <div className="absolute bottom-16 left-0 right-0 bg-white border-t border-zinc-100 rounded-t-2xl shadow-xl px-4 pt-4 pb-6"
            ...
            <div className="grid grid-cols-4 gap-1.5 overflow-y-auto" style={{ maxHeight: 'calc(100vh - 140px)' }}>
              {MORE_ITEMS.map(item => {
                ...
                if (isLogout) {
                  return (
                    <button key="logout"
                      ...
                      className="flex flex-col items-center gap-1.5 px-2 py-2.5 rounded-xl transition-colors text-zinc-500 hover:bg-zinc-50">
                      {item.icon}
                ...
                return (
                  <Link key={item.href} href={item.href!}
                    ...
                    className={`flex flex-col items-center gap-1.5 px-2 py-2.5 rounded-xl transition-colors ${active ? 'bg-amber-50 text-amber-600' : 'text-zinc-500 hover:bg-zinc-50'}`}>
                    {item.icon}
                    <span className="text-[11px] font-medium">{item.label}</span>
                  </Link>
                )
